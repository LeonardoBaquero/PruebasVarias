...
-- Tip: Group by
select country, count(*) as Cantidad from users
group by country;
...
select first_name,
       ...
       last_connection
       from users
where country = ('Iceland') and ((last_connection like '112.%') or (users.last_connection like '28.%') or (users.last_connection like '188.%'));
